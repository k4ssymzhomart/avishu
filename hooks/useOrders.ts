...
function useSubscribedOrders(
  ...
  useEffect(() => {
    const subscribeToOrders = subscribeRef.current;

    if (!subscribeToOrders || !cacheKey) {
      setOrders([]);
      setIsLoading(false);
      return;
    }
    ...
    ensureOrdersSubscription(cacheKey, subscribeToOrders);
